...
export default function Navbar() {
  ...
  // Profile dropdown content — shared between mobile and desktop
      ...
      <div className={`${positionClasses} w-56 bg-navy-900/95 backdrop-blur-xl border border-white/[0.08] rounded-xl overflow-hidden z-[60] shadow-[0_12px_40px_rgba(0,0,0,0.5)]`}>
        {username ? (
          <>
            ...
            {isAdmin && (
              <Link href="/admin" onClick={() => { setProfileOpen(false); setMobileMenuOpen(false); }}
                className="flex items-center gap-2.5 w-full text-left px-4 py-3.5 text-sm text-white/60 hover:bg-white/[0.06] hover:text-flag-gold active:bg-white/[0.08] transition-colors">
            ...
            <button onClick={handleLogout}
              className="flex items-center gap-2.5 w-full text-left px-4 py-3.5 text-sm text-white/60 hover:text-flag-red hover:bg-flag-red/10 active:bg-flag-red/15 transition-colors">
              <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <Link href="/login" onClick={() => { setProfileOpen(false); setMobileMenuOpen(false); }}
            className="flex items-center gap-2.5 w-full text-left px-4 py-4 text-sm text-white/70 hover:text-flag-gold hover:bg-white/[0.06] active:bg-white/[0.08] transition-colors">
            <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-6 lg:px-8 h-16 flex items-center justify-between gap-6">
        ...
        <div className="hidden md:flex items-center gap-4 flex-1 justify-end">
          ...
          <div ref={desktopProfileRef} className="relative">
            ...
            {renderProfileDropdown('absolute right-0 top-full mt-2')}
          </div>
        </div>
