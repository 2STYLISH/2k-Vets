        ...
        <table className="w-full text-left text-sm text-white/70">
          <thead className="bg-navy-800 text-white text-xs font-mono uppercase tracking-widest">
            <tr>
              ...
              <th className="px-5 py-3.5 font-medium text-center">W-L</th>
              <th className="px-5 py-3.5 font-medium text-center">PD</th>
              <th className="px-5 py-3.5 font-medium text-right">PCT</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-navy-100/20">
            {rows.map((r, i) => (
              <tr key={r.team.id} className="hover:bg-white/[0.03] transition-colors">
                <td className="px-5 py-3.5 text-white font-mono font-bold">{i + 1}</td>
                <td className="px-5 py-3.5 text-white/90 font-medium">{r.team.name}</td>
                <td className="px-5 py-3.5 text-center text-white/70">{r.wins} - {r.losses}</td>
                <td className="px-5 py-3.5 text-center text-flag-gold font-bold">{r.pd > 0 ? `+${r.pd}` : r.pd}</td>
                <td className="px-5 py-3.5 text-right font-mono font-bold text-white">{r.winPct}</td>
              </tr>
            ))}
          </tbody>
        </table>
