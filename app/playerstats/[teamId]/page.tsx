...
export default async function TeamDetailPage({ params }: { params: { teamId: string } }) {
  ...
  return (
    <div className="space-y-10">
      {/* Header */}
      <div>
        <Link href="/teams" className="text-xs font-mono text-white/40 hover:text-white transition-colors uppercase tracking-widest">
          ← All Teams
        </Link>
        <h1 className="text-4xl text-white mt-3 mb-1">{team.name}</h1>
        <p className="text-white/40 text-sm">Season 1 · Verified game stats</p>
      </div>

      {/* Season Averages Table */}
      <section>
        <h2 className="text-sm font-mono text-white/30 uppercase tracking-widest mb-3">Season Averages</h2>
            ...
            <table className="w-full text-xs stat-mono">
              ...
              <tbody>
                {playerRows.map(({ player, avg }) => (
                  <tr key={player.id} className="border-b border-surface-800 last:border-0 hover:bg-white/[0.03] transition-colors">
                    <td className="px-5 py-3">
                      <span className="text-silver-200 font-body">{player.gamertag}</span>
                      {player.position && (
                        <span className="ml-2 text-[10px] text-white/40 uppercase">{player.position}</span>
                      )}
                    </td>
                    {avg ? (
                      <>
                        ...
                        <td className="px-3 py-3 text-right text-white/30">{avg.tpPct}%</td>
                        <td className="px-3 py-3 text-right text-white/30">{avg.ftPct}%</td>
                      </>
                    ) : (
      ...
      {gameLogArr.length > 0 && (
        <section>
          <h2 className="text-sm font-mono text-white/30 uppercase tracking-widest mb-3">Game Log</h2>
              ...
              return (
                <div key={game.id} className="card p-5">
                  <div className="flex items-center gap-4 mb-3">
                    <span className={`text-xs font-mono px-2 py-0.5 rounded ${won ? 'bg-white/[0.06] text-silver-200' : 'bg-white/[0.03] text-white/40'}`}>
                      {won ? 'W' : 'L'}
                    </span>
                    <p className="text-white/50 text-sm">
                      {isHome ? 'vs' : '@'} {oppName ?? 'Unknown'}
                    </p>
                    <p className="text-white/40 text-xs font-mono ml-auto">
                      {myScore} – {oppScore}
                    </p>
                  </div>
                  <div className="overflow-x-auto">
                        ...
                        {playerLines.map((line) => {
                          ...
                          return (
                            <tr key={line.player_id} className={`border-b border-surface-900 last:border-0 ${line.did_not_play ? 'opacity-40' : ''}`}>
                              <td className="py-1 pr-3 text-white/30 whitespace-nowrap">
                                {p?.gamertag ?? '—'}
                                {line.did_not_play && <span className="ml-2 text-[9px] text-white/40">DNP</span>}
                              </td>
                              <td className="px-1 text-right text-silver-200">{line.pts}</td>
                              <td className="px-1 text-right text-white/30">{line.reb}</td>
                              <td className="px-1 text-right text-white/30">{line.ast}</td>
                              <td className="px-1 text-right text-white/30">{line.stl}</td>
                              <td className="px-1 text-right text-white/30">{line.blk}</td>
                              <td className="px-1 text-right text-white/30">{line.turnovers}</td>
                              <td className="px-1 text-right text-white/30">{line.fgm}</td>
                              <td className="px-1 text-right text-white/30">{line.fga}</td>
                              <td className="px-1 text-right text-white/30">{line.tpm}</td>
                            </tr>
                          );
                        })}
                      </tbody>
