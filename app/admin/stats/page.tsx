    ...
    <div className="space-y-10">
      ...
      <div className="pb-6 border-b border-white/[0.06]">
        ...
        <div className="flex gap-6 text-sm text-white/40 mb-6">
          ...
          {totalPendingGames > 0 && (
            <span>
              <span className="text-white/30 font-mono">{totalPendingGames}</span> pending review
            </span>
          )}
        </div>
        ...
        return (
          <section key={team.id}>
            <div className="flex items-center gap-4 mb-4">
              ...
              <Link href={`/teams/${team.id}`} className="text-[10px] font-mono text-white/40 hover:text-white transition-colors uppercase tracking-widest">
                Public view →
              </Link>
            </div>
                    ...
                    {rows.map(({ player, verifiedAvg, pendingAvg }) => {
                      ...
                      return (
                        <tr key={player.id} className={`border-b border-surface-800 last:border-b-0 hover:bg-white/[0.03] transition-colors group/row ${isPending ? 'opacity-60' : ''}`}>
                          ...
                          <td className="px-4 py-4 text-right text-white/50">{avg.rpg}</td>
                          <td className="px-4 py-4 text-right text-white/50">{avg.apg}</td>
                          <td className="px-4 py-4 text-right text-white/50">{avg.spg}</td>
                          <td className="px-4 py-4 text-right text-white/50">{avg.bpg}</td>
                          <td className="px-4 py-4 text-right text-white/30">{avg.fgPct}%</td>
                          <td className="px-4 py-4 text-right text-white/30">{avg.tpPct}%</td>
